...
# 定义一个namedtuple类型Hypothesis，并包含value，score属性
Hypothesis = namedtuple('Hypothesis', ['value', 'score'])


class NMT(nn.Module):
    """ Simple Neural Machine Translation Model(简单的神经机器翻译模型):
        - Bidrectional LSTM Encoder(双向LSTM编码器)
        - Unidirection LSTM Decoder(单向LSTM解码器)
        - Global Attention Model (全局注意力模型 Luong, et al. 2015)
    """
    ...
    def forward(self, source: List[List[str]],
        ...
        """取一小批源句和目标句，在 NMT 系统学习的语言模型下计算目标句的对数似然。
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]
        ...
        # pack：打包。将几个 tensor打包成一个，返回一个PackedSequence 对象。经过pack后，RNN可以对不同长度的样本数据进行小批量训练，否则只能单个样本进行训练。
        # torch.cat 可以拼接两个tensor
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) # enc_hiddens: (b, src_len, h*2) dot (h*2, h) -> (b, src_len, h)
        Y = self.model_embeddings.target(target_padded) # (tgt_len, b, e)

        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = torch.squeeze(Y_t) # squeeze Y_t: (1, b, e) -> (b, e)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1) # Ybar_t: (b, e+h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2))  # (b, src_len, h) mat2 (b, h, 1) -> (b, src_len, 1)
        ...
        alpha_t = torch.unsqueeze(F.softmax(e_t, dim=1), dim=1)  # attention vector: (b, 1, src_len)
        a_t = torch.squeeze(torch.bmm(alpha_t, enc_hiddens), 1)# (b, 1, src_len) mat2 (b, src_len, 2h) -> (b, 1, 2h) -> (b, 2h)
